<x-app-layout>
    <x-slot name="header">Registrar Nueva Danza</x-slot>
            ...
            <div class="bg-gray-50/40 px-16 py-12 border-b border-gray-50 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2 ml-6">
                        Define una nueva danza oficial para el catálogo de la Calle Los Andes
                    </p>
                </div>
            </div>

            {{-- FORMULARIO --}}
            <form action="{{ route('admin.danzas.store') }}"
                  method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-4">
                        <label class="text-[11px] font-black uppercase text-gray-400 tracking-[0.3em] ml-2 block">
                            Nombre Oficial de la Danza
                        </label>
                        <input type="text"
                               name="nom_danza"
                               value="{{ old('nom_danza') }}"
                               placeholder="EJ: MORENADA"
                               required
                        ...
                        @error('nom_danza')
                            <p class="text-red-500 text-[10px] font-black mt-2 bg-red-50 p-3 rounded-xl italic border border-red-100">⚠️ {{ $message }}</p>
                        @enderror
                    </div>

                    <div class="space-y-4">
                        <label class="text-[11px] font-black uppercase text-gray-400 tracking-[0.3em] ml-2 block">
                            Clasificación Folklórica
                        </label>
                        <select name="clasificacion"
                                class="w-full rounded-2xl border-gray-100 bg-gray-50/50 font-bold text-sm p-5 focus:ring-andes-verde">
                            <option value="Pesada" {{ old('clasificacion') == 'Pesada' ? 'selected' : '' }}>Pesada</option>
                            <option value="Liviana" {{ old('clasificacion') == 'Liviana' ? 'selected' : '' }}>Liviana</option>
                            <option value="Autóctona" {{ old('clasificacion') == 'Autóctona' ? 'selected' : '' }}>Autóctona</option>
                            <option value="Sátira" {{ old('clasificacion') == 'Sátira' ? 'selected' : '' }}>Sátira</option>
                            <option value="Salón" {{ old('clasificacion') == 'Salón' ? 'selected' : '' }}>Salón</option>
                        </select>
                        @error('clasificacion')
                            <p class="text-red-500 text-[10px] font-black mt-2 bg-red-50 p-3 rounded-xl italic">⚠️ {{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <textarea name="descripcion"
                              rows="6"
                              placeholder="Escribe aquí los detalles, origen y significado de esta danza..."
                              class="w-full rounded-[2.5rem] border-gray-100 bg-gray-50/50 text-base p-8 focus:ring-andes-verde transition-all">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="text-red-500 text-[10px] font-black mt-2 bg-red-50 p-3 rounded-xl italic">⚠️ {{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-16 pt-10 border-t border-gray-50">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="w-full max-w-sm h-64 rounded-[2rem] overflow-hidden border border-gray-100 bg-gray-50 shadow-inner mx-auto md:mx-0 flex items-center justify-center">
                            ...
                            <div id="empty-preview"
                                ...
                                <span class="text-[10px] font-black uppercase">Sin archivo seleccionado</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col justify-center space-y-6">
                        <label class="text-[11px] font-black uppercase text-gray-400 tracking-[0.3em] ml-2 block">
                            Cargar Fotografía Referencial
                        </label>

                        <label class="cursor-pointer group">
                            <div class="flex flex-col items-center justify-center p-10 border-2 border-dashed border-gray-200 rounded-[2rem] bg-gray-50 hover:border-andes-verde hover:bg-white transition-all duration-300">
                                
                                <div class="w-12 h-12 bg-andes-verde/10 rounded-full flex items-center justify-center mb-4 group-hover:bg-andes-verde group-hover:text-white transition-all">
                                ...
                                <p id="file-name" class="text-[10px] text-gray-400 mt-3 font-bold uppercase italic">
                                    JPG, PNG (Máx 2MB)
                                </p>
                            </div>
                ...
                <div class="flex flex-col md:flex-row gap-6 pt-12">
                    <a href="{{ route('admin.danzas.index') }}"
                       class="flex-1 py-6 text-center text-gray-400 font-black uppercase text-[11px] tracking-[0.3em] rounded-2xl border border-gray-100 hover:bg-gray-50 transition-all">
                        Descartar
                    </a>

                    <button type="submit"
                            class="flex-[2] bg-andes-oscuro text-white py-6 rounded-[2rem] font-black uppercase text-[11px] tracking-[0.4em] hover:bg-black hover:scale-[1.02] active:scale-95 transition-all shadow-xl">
