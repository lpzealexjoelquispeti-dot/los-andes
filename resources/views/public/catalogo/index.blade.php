    ...
    <div x-data="{ 
        ...
        <div class="w-full lg:max-w-md relative"
            ...
            @if(isset($terminosRelacionados) && $terminosRelacionados->count())
                <div class="mt-3 px-2">
                    <p class="text-gray-400 text-[9px] font-black uppercase tracking-widest mb-2">Filtrar por subcategoría:</p>
                    <div class="flex flex-wrap gap-2">
                        @foreach($terminosRelacionados as $termino)
                            <a href="{{ route('public.catalogo.index', ['q' => $termino]) }}"
                               class="px-3 py-1 bg-andes-verde/10 border border-andes-verde/20 rounded-full
                                      text-[10px] font-black uppercase text-andes-verde hover:bg-andes-verde
            ...
            @if(!request('q') && isset($populares) && $populares->count())
                <div class="mt-4">
                    <p class="text-gray-400 text-[9px] font-black uppercase tracking-widest mb-2 px-1">Búsquedas frecuentes</p>
                    <div class="flex flex-wrap gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-10">
            @forelse($trajes as $traje)
                @php 
                    ...
                    // ── TALLAS DISPONIBLES ──
                    $trajeJson['tallas'] = $traje->unidades
                    ...
                    <div class="px-8 pb-8 flex items-center justify-between">
                        ...
                        <div class="p-3 bg-gray-50 rounded-xl group-hover:bg-gray-100 transition-colors">
                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-width="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                            </svg>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-span-full py-20 text-center bg-gray-50 rounded-[3rem] border-4 border-dashed border-gray-100">
                    <div class="mb-6 opacity-20 flex justify-center">
                        <svg class="w-20 h-20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-width="1" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                    </div>
        ...
        <div x-show="openModal" 
            ...
            <div class="bg-white w-full max-w-5xl max-h-[90vh] rounded-[3rem] overflow-hidden shadow-2xl relative flex flex-col lg:flex-row" 
                ...
                <div class="w-full lg:w-1/2 p-8 lg:p-12 flex flex-col overflow-y-auto">
                    ...
                    {{-- Precio + Tallas --}}
                    <div class="py-6 border-y border-gray-100 mb-8 bg-gray-50/50 rounded-3xl px-6">
                        <div class="mb-5">
                        ...
                        {{-- ── TALLAS DISPONIBLES ── --}}
                        <div>
                            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-3">Tallas Disponibles</p>
                            <div class="flex flex-wrap gap-2">
                                <template x-for="talla in currentTraje.tallas" :key="talla">
                                    <span class="px-4 py-2 rounded-xl text-xs font-black uppercase border-2 transition"
                                          :style="'border-color:' + currentTraje.color_tienda + '; color:' + currentTraje.color_tienda"
                                          x-text="talla">
                                    </span>
                                </template>
                                <template x-if="!currentTraje.tallas || currentTraje.tallas.length === 0">
                                    <span class="text-gray-400 text-xs font-bold italic">Sin stock disponible</span>
                                </template>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex-grow">
                        ...
                        <p class="text-gray-500 text-sm leading-relaxed italic" 
                           x-text="currentTraje.des_traje || 'Sin descripción adicional detallada.'"></p>
                    </div>
                    ...
                    <div class="mt-10">
                        <a :href="'https://wa.me/591' + currentTraje.whatsapp + '?text=Hola! Vengo del catálogo global. Me interesa alquilar el traje: ' + currentTraje.nom_traje" 
                           ...
                           <x-si-whatsapp class="w-5 h-5 text-white fill-current" /> 
