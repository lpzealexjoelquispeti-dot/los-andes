{{--
    PARTIAL OPTIMIZADO: vendedor/unidades/_perchero.blade.php
--}}
...
<div class="mt-14"
     ...
     {{-- ESCUCHADOR TRANSACCIONAL CLAVE: Recibe la nueva lista del género activo --}}
     @genero-cambiado.window="unidades = $event.detail.unidades">
            ...
            <tbody class="divide-y divide-gray-100">
                <template x-for="u in unidades" :key="u.cod_unidad">
                    <tr class="transition-colors"
                        :class="u.deleted_at ? 'bg-red-50/40 opacity-60 text-gray-400' : 'hover:bg-gray-50/80'">
                        ...
                        <td class="px-5 py-3">
                            <template x-if="u.deleted_at">
                                <span class="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-red-600 text-white">🛑 Baja Lógica</span>
                            </template>
                            <template x-if="!u.deleted_at">
                        ...
                        <td class="px-5 py-3 font-medium italic text-gray-400 truncate max-w-[160px]" :title="u.observaciones">
                            <span x-text="u.deleted_at ? 'Prenda removida del stock' : (u.observaciones || 'Sin novedades')"></span>
                        </td>

                        <td class="px-5 py-3 text-right">
                            <div class="flex items-center justify-end gap-1.5">
                                {{-- Formularios CRUD dinámicos sincronizados por ID --}}
                                <form :id="'delete-form-' + u.cod_unidad" :action="'/vendedor/unidades/' + u.cod_unidad" method="POST" class="hidden">
                                    @csrf @method('DELETE')
                                </form>
                                <form :id="'restore-form-' + u.cod_unidad" :action="'/vendedor/unidades/' + u.cod_unidad + '/restore'" method="POST" class="hidden">
                                    @csrf
                                </form>
                ...
                <template x-if="unidades.length === 0">
                    <tr>
                        <td colspan="5" class="p-10 text-center text-gray-400 font-bold uppercase tracking-widest text-[10px]">No hay códigos físicos para este género.</td>
                    </tr>
                </template>
            </tbody>
        </table>
    ...
    {{-- MODAL DE EDICIÓN (Mantiene tu misma estructura impecable) --}}
    <div x-show="modalEditarOpen" x-transition.opacity class="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm" x-cloak>
        <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden" @click.away="modalEditarOpen = false">
            <div class="bg-andes-oscuro px-8 py-6 flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-base font-black text-white uppercase mt-0.5 font-mono tracking-tight" x-text="editando.serial"></h3>
                    <p class="text-[10px] text-slate-500 mt-1">El código identificador no es editable.</p>
                </div>
                <button type="button" @click="modalEditarOpen = false" class="text-slate-400 hover:text-white transition mt-1 shrink-0">
            ...
            <form :action="'/vendedor/unidades/' + editando.id" method="POST" class="px-8 py-6 space-y-5">
                ...
                @method('PUT')
                <div>
                    <label class="block text-[10px] font-black uppercase text-gray-400 tracking-widest mb-1.5">Talla de la Prenda</label>
                    <select name="talla" x-model="editando.talla" @change="detectarCambioTalla()" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-black uppercase text-gray-700 focus:bg-white focus:border-andes-verde focus:outline-none transition">
                    ...
                    <div x-show="editando.tallaAdvertencia" x-transition class="mt-2.5 flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-3.5 py-3">
                        <svg class="w-4 h-4 text-amber-500 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                        <div>
                            <p class="text-[10px] font-black text-amber-700 uppercase tracking-wide">⚠ Código quedará desincronizado</p>
                            <p class="text-[10px] text-amber-600 mt-0.5 leading-relaxed">El código <strong x-text="editando.serial"></strong> no cambiará automáticamente.</p>
                        </div>
                    </div>
                </div>

                <div>
                    <label class="block text-[10px] font-black uppercase text-gray-400 tracking-widest mb-2">Condición Física Actual</label>
                    <div class="grid grid-cols-2 gap-2">
                        <template x-for="opcion in [
                            ...
                            <label class="flex items-center gap-2 px-3 py-2.5 rounded-xl border-2 cursor-pointer transition-all select-none" :class="editando.estado === opcion.valor ? opcion.activo : 'border-gray-100 bg-gray-50 text-gray-500 hover:border-gray-200'">
                                <input type="radio" name="estado_fisico" :value="opcion.valor" x-model="editando.estado" class="sr-only">
                                ...
                                <svg x-show="editando.estado === opcion.valor" class="w-3.5 h-3.5 ml-auto shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                            </label>
                        </template>
                    </div>
                </div>
                ...
                <div class="flex gap-3 pt-1">
                    <button type="button" @click="modalEditarOpen = false" class="flex-1 py-3 rounded-xl border border-gray-200 text-gray-500 font-black uppercase text-[10px] tracking-widest hover:bg-gray-50 transition">Cancelar</button>
                    <button type="submit" class="flex-1 py-3 bg-andes-oscuro hover:bg-black text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg transition">Guardar Cambios</button>
                </div>
            </form>
        </div>
    </div>
