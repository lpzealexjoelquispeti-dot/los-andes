<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-black text-xl text-gray-800 leading-tight uppercase">
                Editar Variante [{{ $traje->genero }}]: {{ str_replace([' - Varón', ' - Damas'], '', $traje->nom_traje) }}
            </h2>
    ...
    <div class="max-w-5xl mx-auto py-8 px-4" x-data="trajeEdit()">
        ...
        <form action="{{ route('vendedor.trajes.update', $traje->cod_traje) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-xl border-t-4 {{ $traje->genero === 'Femenino' ? 'border-pink-500' : ($traje->genero === 'Masculino' ? 'border-blue-500' : 'border-gray-700') }}">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-4">
                            ...
                            @foreach($traje->imagenes as $img)
                                <div id="imagen_existente_{{ $img->cod_imagen }}" 
                                    ...
                                    <span class="absolute bottom-1 left-1 bg-white/80 text-[7px] font-black px-1.5 rounded-md uppercase text-gray-500 shadow-sm">Actual</span>
                                </div>
                            @endforeach

                            {{-- Renderizado de Previsualizaciones de Fotos Nuevas --}}
                            ...
                            <label x-show="totalPhotosCount() < 7" 
                                ...
                                <svg class="w-8 h-8 text-gray-400 group-hover:text-andes-verde transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        ...
                        <div class="text-center">
                            <p class="text-xs font-bold uppercase tracking-tighter text-gray-400">
                                <span :class="(totalPhotosCount() < 4 || totalPhotosCount() > 7) ? 'text-andes-rojo' : 'text-andes-verde'" x-text="totalPhotosCount()"></span> de 7 fotos totales
                            </p>
                            <p class="text-[9px] font-bold text-gray-400 uppercase mt-1">(Exigido mínimo 4 piezas)</p>
                        </div>
                    </div>
                </div>

                {{-- ── PANEL DERECHO: FORMULARIO DE EDICIÓN DE VALORES ── --}}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2 bg-gray-50 p-4 rounded-xl border border-gray-200">
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                                    @php
                                        ...
                                        $tallasGuardadas = $traje->unidades->pluck('talla')->toArray();
                                    @endphp
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="des_traje" name="des_traje" rows="4" 
                                          class="block mt-1 w-full border-gray-300 focus:border-andes-verde focus:ring-andes-verde rounded-lg shadow-sm" placeholder="Detalla los componentes específicos de este género...">{{ old('des_traje', $traje->des_traje) }}</textarea>
                            </div>
                        </div>
            ...
            {{-- Almacén Oculto de Fotos Borradas --}}
