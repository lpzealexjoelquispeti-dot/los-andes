...
<style>
    ...
    .btn-primary {
        font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: .7rem;
...
<div class="v-font max-w-5xl mx-auto px-4 py-10">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4 mb-8">
        <div>
            ...
            <h1 class="v-title text-2xl lg:text-3xl text-gray-900">{{ $traje->nom_traje }}</h1>
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="info-block flex gap-4 fade-in">
                ...
                <div class="flex-1">
                    ...
                    <p class="v-title text-lg text-gray-900">{{ $traje->nom_traje }}</p>
                ...
                <div class="text-right">
                    ...
                    <p class="v-title text-2xl text-gray-900">Bs. {{ number_format($alquiler->monto_total, 0) }}</p>
                    <p class="text-[10px] font-semibold text-green-600 mt-0.5">Seña: Bs. {{ number_format($alquiler->monto_sena, 0) }}</p>
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    @if($alquiler->fec_retorno_real)
                        <div>
                            ...
                            <p class="info-value text-green-700">{{ \Carbon\Carbon::parse($alquiler->fec_retorno_real)->format('d/m/Y') }}</p>
                        </div>
                    @endif
                </div>
            </div>
                        ...
                        @foreach($alquiler->sanciones as $sancion)
                            <div class="flex items-center justify-between py-2 border-b border-red-100 last:border-0">
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-[9px] font-bold {{ $sancion->pagada ? 'text-green-600' : 'text-red-500' }}">
            ...
            <div class="space-y-3 fade-in">
                ...
                @if($alquiler->est_alquiler === 'Pendiente_Aprobacion')
                    <form method="POST" action="{{ route('vendedor.alquileres.aprobar', $alquiler->cod_alquiler) }}">
                        @csrf @method('PATCH')
                        <button type="submit" class="btn-primary w-full" style="background:#16a34a">
                            ✓ Aprobar solicitud
                        </button>
                    </form>
                    ...
                    <div x-data="{ showRechazo: false }">
                        <button type="button" @click="showRechazo = !showRechazo"
                                class="btn-outline w-full border-2 border-red-200 text-red-600 hover:bg-red-50 hover:border-red-400">
                            ✗ Rechazar solicitud
                        </button>
                        <div x-show="showRechazo" x-cloak x-transition class="mt-3">
                            <form method="POST" action="{{ route('vendedor.alquileres.rechazar', $alquiler->cod_alquiler) }}"
                                ...
                                <textarea name="motivo_rechazo" required rows="3"
                                          ...
                                          class="w-full border-2 border-gray-200 rounded-xl p-3 text-sm font-medium resize-none focus:border-red-400 focus:ring-0 outline-none"></textarea>
                                <button type="submit" class="btn-primary w-full" style="background:#dc2626">
                                    Confirmar rechazo
                                </button>
                            </form>
                        </div>
                ...
                @if($alquiler->est_alquiler === 'Reservado')
                    <form method="POST" action="{{ route('vendedor.alquileres.entregar', $alquiler->cod_alquiler) }}">
                        @csrf @method('PATCH')
                        <button type="submit" class="btn-primary w-full" style="background:#2563eb">
                ...
                @if($alquiler->est_alquiler === 'Entregado')
                    <form method="POST" action="{{ route('vendedor.alquileres.devolver', $alquiler->cod_alquiler) }}">
                        @csrf @method('PATCH')
                        <button type="submit" class="btn-primary w-full" style="background:#7c3aed">
